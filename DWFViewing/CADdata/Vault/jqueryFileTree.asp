<%
  ...
  if len(BaseFileDir)=0 then BaseFileDir="DWFs/"
  ...
  regEx.Global = True
  regEx.Pattern = "%20"
  BaseFileDir = regEx.Replace(BaseFileDir, Chr(32))
  regEx.Pattern = "%28"
  BaseFileDir = regEx.Replace(BaseFileDir, "(")
  regEx.Pattern = "%29"
  BaseFileDir = regEx.Replace(BaseFileDir, ")")
  ...
  if ObjFSO.FolderExists(BaseFile) then
    ...
    For Each ObjFile In ObjFolder.Files
      ...
      Html = Html + "<li class=""file ext_"&i__Ext&""">"&_
        "<a href=""#"" rel="""+(BaseFileDir+i__Name)+""">"&_
        Left(i__name, Len(i__name)-1-Len(i__Ext))+"</a></li>"&VBCRLF
    Next
    Html = Html +  "</ul>"&VBCRLF
  end if
